...
class ListItem extends React.Component {
  ...
  render() {
    ...
    return (<ul aria-label={'A list of Locations where you can find some of Kansas City\'s finest barbecue'}>
      {
        restaurants.map(place => {
          return (<li id={place.id} className={this.state.isListItemOpen && this.state.listItem === place.id
              ? 'restaurant-list-item'
              : 'restaurant-list-item-closed'} key={place.id} tabIndex='0' onClick={this.handleListClick} js-data={place.id} aria-label={`${place.name}`} aria-expanded={this.state.isListItemOpen
              ? 'true'
              : 'false'}>

            <img js-data={place.id} aria-hidden={this.state.isListItemOpen
                ? 'true'
                : 'false'} className={this.state.isListItemOpen && this.state.listItem === place.id
                ? 'list-image'
                : 'hide-image'} src={!place.bestPhoto.prefix
                ? noImage
            ...
            <div className="list-info-wrapper" js-data={place.id}>
              ...
              <h5 className="list-title" js-data={place.id}>{
